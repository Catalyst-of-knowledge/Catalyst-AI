...
## 📊 R (Results: 結果)
【重要: 最も詳細に記述するセクションです。以下の要素を絶対に省略せず、全て抽出すること】
...
抽象的な表現（例：「〜について述べられている」「〜が調査された」）は避け、実際に「何が分かり」「どういうデータが出たのか」を明記すること。
...
## 💡 主要な発見とデータ（重要度の高い順に）
- (具体的な数値や事実に基づく発見を深掘りして箇条書き)
- (抽象的な言葉を避け、事実と結果を記載)

## ⚙️ 採用されたアプローチ・根拠
- (どのような手法・データに基づいてその結論に至ったか)
